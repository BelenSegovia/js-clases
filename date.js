'use strict'
/*
const fecha= new Date(2022,7,10,15,25,36);
const fechaString= new Date(`2025-08-10`);
//10/08
console.log(fecha);
...
class Tarea {
    constructor(titulo, descripcion, fechaVenc){
        ...
        //lo pasamos como objeto date y deje de ser de tipo string
        // aaaa/mm/dd
...
class GestorTareas {
    constructor(limite){
        this.tareas = [];
        this.limite = Number(limite) || 10; // toma por defecto el valor de 10
    }
    ...
    existeTarea(tituloABuscar){
       ...
       const coincidencia= this.tareas.find((item)=>{
            //el arreglo encuentra la coincidencia cuando:
            return item.titulo.toLowerCase() === tituloABuscar.toLowerCase();
        })
        if (coincidencia){
            console.log('La tarea existe: ', coincidencia);
        } else {
